...
const FUND='HABITAT';
const ORIGIN='VISOR GITHUB · HABITAT · NIVELES/RETORNOS';
const TRADE_KEY='habitat_fondo3_trade_history_v3';
const LEGACY_KEYS=['habitat_fondo3_trade_history_v2','fondo3_trade_history_v1','habitat_fondo3_trade_history_v1'];
...
function installPanel(){
  ...
  <div class="tradehead"><div><h2 style="margin:0">Mis operaciones · Hábitat Fondo 3</h2><div class="sub">Mismo registro y sincronización con tu hoja “Operaciones Fondo 3” en Drive. Las nuevas operaciones pueden valorarse con Niveles o Retornos.</div></div><div class="small" id="tradeCloudBadge">Drive: verificando…</div></div>
...
async function syncNow(){
  ...
  try{
    const probe=await jsonp('ping');if(probe.routing!==true||String(probe.fund||'').toUpperCase()!==FUND)throw new Error('El puente no está enroutando a la hoja Hábitat');
    ...
    localStorage.setItem(TRADE_KEY,JSON.stringify(remote));localStorage.setItem(SNAP_KEY,JSON.stringify(remote));cloudStatus(`Drive Hábitat conectado · ${remote.length} ${remote.length===1?'operación':'operaciones'}.`,'cloud-ok');renderHistory();
...
async function boot(){
  ...
  try{await loadData();refreshDateInputs(true);renderHistory();cloudStatus(c.key?'Drive Hábitat configurado. Verificando…':'Drive listo: se conserva la misma conexión anterior; ingresa tu clave solo si este navegador no la conserva.',c.key?'cloud-warn':'');if(c.key)setTimeout(syncNow,500)}
