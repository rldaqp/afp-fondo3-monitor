...
def alinear_nivel(
    ...
    """
    Alinea cada fecha de cuota con el último cierre de mercado disponible.

    Corrección:
    yfinance puede devolver el índice con nombres como "Date", "Datetime"
    o "fecha_mercado". Por eso no se asume que la columna creada por
    reset_index() se llame siempre "index".
    """
    ...
    if mercado.shape[1] < 2:
        return pd.Series(
            ...
            index=pd.to_datetime(
                fechas_cuota["fecha_cuota"], errors="coerce"
            ).dt.normalize(),
            name="nivel",
        )

    # La primera columna siempre corresponde al antiguo índice de fechas.
    primera_columna = mercado.columns[0]
    mercado = mercado.rename(columns={primera_columna: "fecha_mercado"})
    ...
    try:
        ...
        pass
    mercado["fecha_mercado"] = mercado["fecha_mercado"].dt.normalize()
    mercado["nivel"] = pd.to_numeric(mercado["nivel"], errors="coerce")
    ...
    try:
        cuotas["fecha_cuota"] = cuotas["fecha_cuota"].dt.tz_localize(None)
    except (TypeError, AttributeError):
        pass
    cuotas["fecha_cuota"] = cuotas["fecha_cuota"].dt.normalize()
    cuotas = cuotas.dropna(subset=["fecha_cuota"]).sort_values("fecha_cuota")
    ...
    alineado = pd.merge_asof(
        ...
        direction="backward",
        tolerance=pd.Timedelta(days=TOLERANCIA_MERCADO_DIAS),
    )
