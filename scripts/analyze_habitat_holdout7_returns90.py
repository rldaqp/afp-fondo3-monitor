...
OUT = Path('analysis/habitat_holdout7_returns90.json')
PUBLIC = Path('public/habitat/data/holdout7_returns90.json')
...
def main():
    ...
    bR90,r2R90,adjR90,fitR90=ols(train90,RET_FACTORS,'ret_vc_sbs')
    ...
    out={
      ...
      'logic':'Mismo holdout de 7 últimas fechas SBS. Niveles 90 y Retornos 90 usan las 90 observaciones previas al 25/08; Retornos 30 usa las 30 observaciones previas. Ningún modelo usa las 7 fechas de validación para estimar coeficientes. Retornos aplica el retorno estimado al VC SBS real de la rueda anterior.',
      'holdout':{
      ...
      'returns90':{
        'training':{'start':train90.fecha.min().strftime('%Y-%m-%d'),'end':train90.fecha.max().strftime('%Y-%m-%d'),'n':int(len(train90))},
        'r2_train':r2R90,'adj_r2_train':adjR90,
        'coefficients':dict(zip(['intercept']+RET_FACTORS,[float(x) for x in bR90])),
        'train_return_mae_pct':float(np.mean(np.abs((fitR90-train90.ret_vc_sbs.to_numpy())*100))),
        'holdout_metrics':metrics(hold.vc_sbs,hold.vc_retornos90),
      },
      'rows':rows,
    }
