...
CACHE = ROOT / "data" / "rolling90" / "bcrp_pd04640_cache.csv"
...
def bcrp_from_html() -> list[tuple[str, float]]:
    r = requests.get(
        BCRP_HTML_URL,
        timeout=30,
        headers={"User-Agent": "Mozilla/5.0", "Accept-Language": "es-PE,es;q=0.9,en;q=0.8"},
    )
    r.raise_for_status()
...
def load_bcrp() -> tuple[list[tuple[str, float]], str]:
    ...
    for name, fn in (("HTML", bcrp_from_html), ("API", bcrp_from_api)):
        try:
            ...
            if rows:
                write_cache(rows)
                return rows, f"BCRP {BCRP_SERIES} · SBS VENTA · {name}"
    ...
    if cache:
        ...
        return cache, f"BCRP {BCRP_SERIES} · SBS VENTA · CACHE OFICIAL"
    raise RuntimeError(f"No se pudo obtener BCRP {BCRP_SERIES}: " + " | ".join(errors))
...
def tucambista_midpoint(signal_date: str) -> tuple[float, str]:
    ...
    for pb, ps in patterns:
        ...
        if buy and sell:
            compra = float(buy.group(1).replace(",", "."))
            venta = float(sell.group(1).replace(",", "."))
...
def operational_fx(signal_date: str) -> dict:
    """FX común para ambos Rolling 30.

    Regla:
    - Si SBS Venta tiene dato oficial para la fecha, se usa ese valor.
    - Si la fecha ya está dentro del histórico publicado pero aparece n.d., se
      arrastra el último SBS Venta oficial. Así los huecos históricos no se
      sustituyen por otra fuente.
    - TuCambista se usa únicamente para una fecha posterior a la última fecha
      publicada por BCRP, mientras el dato oficial aún no existe.
    """
    bcrp, bcrp_method = load_bcrp()
    if not bcrp:
        raise RuntimeError("Serie SBS Venta vacía")

    prior = [(d, v) for d, v in bcrp if d < signal_date]
    if not prior:
        raise RuntimeError(f"No existe SBS Venta previo a {signal_date}")
    ...
    if same:
        ...
        return {
            ...
            "source": bcrp_method + " · TC SISTEMA BANCARIO SBS VENTA · OFICIAL",
            "provisional": False,
        }

    latest_date = bcrp[-1][0]
    if signal_date <= latest_date:
        ...
        return {
            ...
            "previous_date": prev_date,
            "return": 0.0,
            "source": bcrp_method + f" · N.D. · ARRASTRE ÚLTIMO OFICIAL {prev_date}",
            "provisional": False,
        }
    ...
    return {
        ...
        "source": label + f" · PROVISIONAL · BASE {bcrp_method} {prev_date}",
        "provisional": True,
    }
...
def replace_fx_asset(model: dict, asset_name: str, fx: dict) -> dict:
    ...
    if row is None:
        ...
        assets.append(row)
    row.update({
        "ticker": BCRP_SERIES if not fx["provisional"] else "TUCAMBISTA",
...
def recalc_model(model: dict, fx: dict, is_new: bool) -> tuple[float, float]:
    ...
    mapping = (
        {"ret_.INX": ".INX", "ret_CPER": "CPER", "ret_EEM_alt": "EEM", "ret_NDX": "NDX", "ret_SPBLSCUP": "SPBLSCUP", "ret_USD_PEN_alt": "USD/PEN"}
        if is_new
        else {"ret_SPY": "SPY", "ret_EEM": "EEM", "ret_EPU": "EPU", "ret_MCHI": "MCHI", "ret_USD_PEN": "USD_PEN", "ret_QQQ": "QQQ"}
    )
    ...
    if target is None or not finite(target.get("base_vc")):
        raise RuntimeError(f"Falta fila/base forward {signal_date} para {model.get('name')}")
    ...
    model["source_note"] = (
        ("Modelo B: .INX, CPER, EEM, NDX, SPBLSCUP y USD/PEN. " if is_new else "Modelo A: SPY, EEM, EPU, MCHI, QQQ y USD/PEN. ")
        + f"Regla FX común: BCRP {BCRP_SERIES} SBS Venta. Si la fecha histórica figura n.d., se arrastra el último valor oficial; TuCambista solo se usa provisionalmente cuando la fecha es posterior a la última publicación BCRP. Yahoo PEN=X no entra en los dos Rolling 30."
    )
    return rr, est
...
def main() -> None:
    ...
    dual["shared_fx_operational"] = {
        **fx,
        "rule": (
            f"BCRP {BCRP_SERIES} SBS Venta oficial. En fechas históricas n.d. se arrastra el último valor oficial; "
            "TuCambista midpoint solo es provisional para fechas posteriores a la última publicación BCRP. Mismo USD/PEN para ambos Rolling 30; Yahoo PEN=X excluido."
        ),
    }
    dual["comparison"] = {
