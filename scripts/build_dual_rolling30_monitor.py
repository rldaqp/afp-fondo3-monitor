...
def qqq_snapshot(signal_date: pd.Timestamp, qdaily: pd.DataFrame, market_open: bool) -> dict:
    ...
    return {
        "serie":"QQQ", "ticker":"QQQ", "timestamp":stamp,
        "precio_anterior":prev_close, "precio_actual":current,
        "retorno":current/prev_close-1.0, "retorno_modelo":current/prev_close-1.0,
        "estado":source, "usado_modelo":True,
    }
...
def one_step_history(common: pd.DataFrame, features: list[str], limit: int=140) -> list[dict]:
    ...
    for i in range(TRAIN, len(common)):
        ...
        train=common.iloc[max(0,i-TRAIN):i].copy()
        if len(train)!=TRAIN: continue
        beta,_=fit(train,features)
        rr=predict(beta,target,features)
        est=float(target["prev_vc"])*(1.0+rr)
        actual=float(target["valor_cuota"])
        rows.append({
            "fecha":pd.Timestamp(target["fecha"]).date().isoformat(),
            "base_date":pd.Timestamp(target["prev_date"]).date().isoformat(),
            "base_vc":float(target["prev_vc"]),
            "vc_estimated":est,
            "actual_vc":actual,
            "return_estimated":rr,
            "actual_return":actual/float(target["prev_vc"])-1.0,
            "signal":classify(rr),
...
def history_metrics(rows: list[dict]) -> dict:
    ...
    e=np.array([r["vc_estimated"]-r["actual_vc"] for r in rows],dtype=float)
    a=np.array([r["actual_vc"] for r in rows],dtype=float)
...
def forward_chain(common: pd.DataFrame, factors: pd.DataFrame, features: list[str], sbs: pd.DataFrame, signal_date: pd.Timestamp) -> tuple[list[dict],dict]:
    latest=sbs.iloc[-1]
    anchor_date=pd.Timestamp(latest["fecha"]).normalize(); vc=float(latest["valor_cuota"])
    train=common.loc[common["fecha"]<=anchor_date].tail(TRAIN).copy()
    if len(train)!=TRAIN:
        raise RuntimeError(f"Train rolling30 incompleto al ancla {anchor_date.date()}: {len(train)}")
    beta,coeff=fit(train,features)
    hidden=factors.loc[(factors["fecha"]>anchor_date)&(factors["fecha"]<=signal_date),["fecha",*features]].dropna(subset=features).sort_values("fecha")
    rows=[]
    for _,r in hidden.iterrows():
        rr=predict(beta,r,features); base=vc; vc=base*(1.0+rr)
        rows.append({"fecha":pd.Timestamp(r["fecha"]).date().isoformat(),"base_vc":base,"vc_estimated":vc,"return_estimated":rr,"signal":classify(rr),"actual_vc":None})
    if not rows and anchor_date==signal_date:
        hist=one_step_history(common,features,1)
        if hist: rows=[{**hist[-1],"actual_vc":float(latest["valor_cuota"])}]
    current=rows[-1] if rows else {"fecha":signal_date.date().isoformat(),"base_vc":float(latest["valor_cuota"]),"vc_estimated":float(latest["valor_cuota"]),"return_estimated":0.0,"signal":"NEUTRO","actual_vc":float(latest["valor_cuota"])}
    meta={"anchor_date":anchor_date.date().isoformat(),"anchor_vc":float(latest["valor_cuota"]),"train_start":pd.Timestamp(train.iloc[0]["fecha"]).date().isoformat(),"train_end":pd.Timestamp(train.iloc[-1]["fecha"]).date().isoformat(),"train_n":len(train),"coefficients":coeff,"blind_chain_sessions":len(rows)}
    return rows,meta


def qqq_intraday_assets(live: dict, qqq: dict) -> list[dict]:
    wanted=["SPY","EEM","EPU","MCHI","USD_PEN"]
...
def new_intraday_values(live: dict, signal_date: pd.Timestamp, new_hist: pd.DataFrame) -> tuple[dict[str,float],list[dict]]:
    ...
    vals={}; assets=[]
    hist_same=new_hist.loc[new_hist["fecha"].eq(signal_date)]
    for f,n in mapping.items():
        row=dict(by.get(n,{"serie":n}))
        ...
        if not finite(raw):
            raise RuntimeError(f"Falta {n} para rolling30 nuevos tickers")
...
def extend_with_live(factors: pd.DataFrame, signal_date: pd.Timestamp, values: dict[str,float], features:list[str]) -> pd.DataFrame:
    row={"fecha":signal_date,**values}
    x=pd.concat([factors,pd.DataFrame([row])],ignore_index=True)
...
def load_blind3() -> dict:
    ...
    try:
        ...
        common=b.get("common_exact_comparison",{}).get("3",{})
        full=b.get("full_six_month_residual",{}).get("3",{})
        return {
            "rule":b.get("blind_rule"),
            "common_period":{"start":common.get("date_start"),"end":common.get("date_end")},
            "qqq_common":common.get("raw_qqq",{}),
            "new_tickers_common":common.get("new_tickers",{}),
            "qqq_full_six_month":full.get("raw_qqq",{}),
            "pairwise":common.get("pairwise",{}),
        }
    except Exception as exc:
...
def main() -> None:
    ...
    sbs=sbs_frame(); latest_sbs=sbs.iloc[-1]
    markets=read_csv(DATA/"markets.csv")
    qdaily=load_qqq_daily(markets["fecha"].min(),signal_date)
    qqq_now=qqq_snapshot(signal_date,qdaily,bool(live.get("market_open")))
    ...
    qf_live=extend_with_live(qf,signal_date,qlive,QQQ_FEATURES)
    qcommon=build_common(sbs,qf,QQQ_FEATURES)

    nf=load_new_factors()
    nlive,nassets=new_intraday_values(live,signal_date,nf)
    nf_live=extend_with_live(nf,signal_date,nlive,NEW_FEATURES)
    ncommon=build_common(sbs,nf,NEW_FEATURES)

    qhist=one_step_history(qcommon,QQQ_FEATURES)
    nhist=one_step_history(ncommon,NEW_FEATURES)
    qforward,qmeta=forward_chain(qcommon,qf_live,QQQ_FEATURES,sbs,signal_date)
    nforward,nmeta=forward_chain(ncommon,nf_live,NEW_FEATURES,sbs,signal_date)

    blind=load_blind3(); qcur=qforward[-1]; ncur=nforward[-1]
    qcur={**qcur,**qmeta}; ncur={**ncur,**nmeta}

    qm=blind.get("qqq_common",{}); nm=blind.get("new_tickers_common",{})
    winner="—"
    if finite(qm.get("mape_pct")) and finite(nm.get("mape_pct")):
        winner="NUEVOS TICKERS" if float(nm["mape_pct"])<float(qm["mape_pct"]) else "QQQ"

    payload={
        "generated_at_lima":pd.Timestamp.now(tz=LIMA).isoformat(),
        "fund":"PROFUTURO Fondo 3",
        "signal_date":signal_date.date().isoformat(),
        "market_mode":live.get("mode"),
        "market_open":bool(live.get("market_open")),
        "latest_sbs":{"fecha":pd.Timestamp(latest_sbs["fecha"]).date().isoformat(),"vc":float(latest_sbs["valor_cuota"])},
        "rule":"Solo quedan dos modelos visibles. Ambos son OLS rolling 30 y se recalibran con las últimas 30 observaciones conocidas. Si SBS está atrasada, se encadenan los días faltantes sin usar los VC ocultos; cuando SBS publica, el siguiente cálculo vuelve a anclarse automáticamente al VC real.",
        "models":{
            "qqq":{
                "key":"qqq","name":"Rolling 30 · QQQ","short":"QQQ","features_display":["SPY","EEM","EPU","MCHI","USD/PEN","QQQ"],
                "features":QQQ_FEATURES,"current":qcur,"forward_chain":qforward,"history_one_step":qhist,"history_metrics":history_metrics(qhist),"intraday_assets":qqq_intraday_assets(live,qqq_now),
                "source_note":"SPY/EEM/EPU/MCHI y QQQ: Yahoo Finance; USD/PEN: fuente operativa del monitor. Entrenamiento histórico usa la serie guardada del modelo.",
            },
            "new_tickers":{
                "key":"new_tickers","name":"Rolling 30 · nuevos tickers","short":"Nuevos tickers","features_display":[".INX","CPER","EEM","NDX","SPBLSCUP","USD/PEN"],
                "features":NEW_FEATURES,"current":ncur,"forward_chain":nforward,"history_one_step":nhist,"history_metrics":history_metrics(nhist),"intraday_assets":nassets,
                "source_note":"Histórico: retornos exactos guardados de Google Finance para .INX, CPER, EEM, NDX y SPBLSCUP; USD/PEN histórico del modelo. En intradía se muestran las fuentes de cada activo y Google Selenium cuando está disponible.",
            },
        },
        "blind3":blind,
        "comparison":{"winner_blind3_mape":winner,"vc_difference":float(ncur["vc_estimated"])-float(qcur["vc_estimated"]),"return_difference":float(ncur["return_estimated"])-float(qcur["return_estimated"])},
    }
