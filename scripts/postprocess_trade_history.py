...
(function(){
  ...
  function entryEffective(requested){
    ...
    return ops.find(x=>x.fecha>=requested)||null;
  }
  function exitEffective(requested){
    if(live&&live.market_open&&live.signal_date===requested)return {fecha:requested,vc:live.vc_estimated,fuente:'MODELO OLS · INTRADÍA PROVISIONAL'};
    return [...ops].reverse().find(x=>x.fecha<=requested)||null;
  }
  ...
  function saveCurrent(){
    ...
    if(!finite(entryEst)){$('tradeMsg').textContent='Todavía no existe un VC estimado para la fecha efectiva de entrada.';return}
    ...
    if(mode==='closed'){
      ...
      if(!finite(exitEst)){$('tradeMsg').textContent='Todavía no existe un VC estimado para la fecha efectiva de salida.';return}
    }
    const rows=loadRows();
...
print("Histórico de entradas/salidas v1 inyectado: P/L por operación + fila TOTAL + conciliación SBS + Drive.")